...
from dalle2_pytorch_code.tokenizer import tokenizer
from dalle2_pytorch_code.vqgan_vae import NullVQGanVAE, VQGanVAE
...
def resize_image_to(
    ...
    **kwargs
    ...
    if not nearest:
        ...
        out = F.interpolate(image, target_image_size, **kwargs)
    else:
...
class Decoder(nn.Module):
    ...
    def p_sample_loop_ddpm(
        ...
        img = (1 - base_noise_level) * lowres_cond_img \
            + base_noise_level * torch.clamp(torch.randn(shape, device = device), -1, 1).where(lowres_cond_img<-0.99, -1)
        ...
        for time in tqdm(
            reversed(range(0, noise_scheduler.num_timesteps)), desc = 'sampling loop time step', total = noise_scheduler.num_timesteps
            ):

            if time > start_diffusion_step: # quick hack
                continue
    ...
    @torch.no_grad()
    def p_sample_loop(self, *args, noise_scheduler, start_diffusion_step, timesteps = None, **kwargs):
        ...
        assert start_diffusion_step <= timesteps

        if not is_ddim:
            return self.p_sample_loop_ddpm(*args, noise_scheduler = noise_scheduler, start_diffusion_step=start_diffusion_step, **kwargs)
    ...
    def sample(
        ...
        if start_at_unet_number > 1:
            ...
            img = resize_image_to(image, prev_unet_output_size, nearest = False, clamp_range=(0, 1), mode='area')
        is_cuda = next(self.parameters()).is_cuda

        num_unets = self.num_unets
        cond_scale = cast_tuple(cond_scale, num_unets)

        for unet_number, unet, vae, channel, image_size, predict_x_start, learned_variance, noise_scheduler, lowres_cond, sample_timesteps, unet_cond_scale \
              in tqdm(zip(range(1, num_unets + 1), self.unets, self.vaes, self.sample_channels, self.image_sizes, self.predict_x_start, self.learned_variance, self.noise_schedulers, self.lowres_conds, self.sample_timesteps, cond_scale)):
            ...
            with context:
                ...
                if unet.lowres_cond:
                    lowres_cond_img = resize_image_to(img, target_image_size = image_size, clamp_range = self.input_image_range, nearest=False, mode='area')

                    # if lowres_cond.use_noise:
                    #     lowres_noise_level = torch.full((batch_size,), int(self.lowres_noise_sample_level * noise_level), dtype = torch.long, device = self.device)
